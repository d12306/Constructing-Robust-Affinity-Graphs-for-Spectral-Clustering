...
% path_stat - the path statistics in all trees by each sample
% Not yet implemented
% proximity

function [Y_new, votes, prediction_per_tree, proximity_ts, nodes, path_stat] = classRF_predict(X, model, extra_options)
    ...
    if exist('extra_options','var')
		...
		% the length (number of nodes) of path in each tree
		if isfield(extra_options, 'path_length') 
            path_length = extra_options.path_length;
        end
    end
	...
	if(~exist('path_length', 'var'))
		path_length = 0; 
	end
    
	[Y_hat, prediction_per_tree, votes, proximity_ts, nodes, path_stat] = mexClassRF_predict(X', model.nrnodes, model.ntree, model.xbestsplit, model.classwt, model.cutoff, model.treemap, model.nodestatus, model.nodeclass, model.bestvar, model.ndbigtree, model.nclass, predict_all, proximity, nodes, path_length);
